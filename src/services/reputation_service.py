...
class ReputationService:
    def rate_response(self, response_id: str, rating: str, user_id: str):
        ...
        if response_details.get("report_owner_id") != user_id:
            raise PermissionError("User is not the owner of the report.")

        response_type = response_details.get("type")
        author_id = response_details.get("author_id")

        # 2. Calculate reputation change
        delta = 0
        is_useful = False
        if response_type == "sighting" and rating == "useful":
            delta = SIGHTING_USEFUL
            is_useful = True
        elif response_type == "finding":
            if rating == "useful":
        ...
        update_result = user_service_client.update_user_reputation(author_id, delta)
        if not update_result:
            raise ConnectionError("Could not update user reputation.")

        # 4. Mark response as rated
        status_result = report_service_client.update_response_status(response_id, is_useful)
